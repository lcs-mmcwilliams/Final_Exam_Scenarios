...
open class Canvas : CustomPlaygroundQuickLookable {
    ...
    open func drawText(message: String, size: Int = 24, x: Int = 0, y: Int = 0, kerning : Float)  {
        ...
        let attributes: [NSAttributedStringKey : AnyObject] = [
            ...
            NSAttributedStringKey.kern: NSNumber(value: kerning) as AnyObject
        ]
    ...
    //        //        let fieldFont = NSFont(name: "Berthold Akzidenz Grotesk Medium", size: CGFloat(size))
    //        let fieldFont = NSFont(name: "Helvetica Bold", size: CGFloat(size))
    //
    //        // set the line spacing to 1
    //        let paraStyle = NSMutableParagraphStyle()
    ...
    //            NSKernAttributeName: NSNumber(value: kerning) as AnyObject
    //        ]
    //
    //        // Draw the string
    //        string.draw(at: NSPoint(x: x, y: y), withAttributes: attributes)
